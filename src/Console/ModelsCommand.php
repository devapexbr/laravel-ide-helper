...
\n\n";
    ...
    public function getPropertiesFromMethods($model)
    {
        ...
        //Edit: Get Protected Methods:
        $reflec_class = new ReflectionClass($model::class);
        $methods = $reflec_class->getMethods();
        ...
        if ($methods) {
            ...
            foreach ($methods as $method) {
                ...
                if (
                    Str::startsWith($method_name, 'get') && Str::endsWith(
                        $method_name,
                        'Attribute'
                    ) && $method_name !== 'getAttribute'
                ) {
                    //Magic get<name>Attribute
                    $name = Str::snake(substr($method_name, 3, -9));
                    if (!empty($name)) {
                        $reflection = new \ReflectionMethod($model, $method_name);
                    ...
                    Str::startsWith($method_name, 'set') && Str::endsWith(
                        $method_name,
                        'Attribute'
                ...
                } elseif (Str::startsWith($method_name, 'scope') && $method_name !== 'scopeQuery') {
                    //Magic set<name>Attribute
                    $name = Str::camel(substr($method_name, 5));
                    if (!empty($name)) {
                        $reflection = new \ReflectionMethod($model, $method_name);
                        $comment = $this->getCommentFromDocBlock($reflection);
                ...
                } elseif (in_array($method_name, ['query', 'newQuery', 'newModelQuery'])) {
                    ...
                    !method_exists('Illuminate\Database\Eloquent\Model', $method_name)
                    && !Str::startsWith($method_name, 'get')
                ) {
                    ...
                    foreach (
                        ...
                        if (stripos($code, $search) || ltrim($impl, '\\') === ltrim((string)$type, '\\')) {
                            ...
                            $methodReflection = new \ReflectionMethod($model, $method_name);
                            ...
                            $relationObj = Relation::noConstraints(function () use ($model, $method_name) {
                                try {
                                    return $model->$method_name();
                                } catch (Throwable $e) {
                                    $this->warn(sprintf('Error resolving relation model of %s:%s() : %s', get_class($model), $method_name, $e->getMessage()));
